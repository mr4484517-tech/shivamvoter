...
# MongoDB Connection
MONGO_URL = "mongodb+srv://mayank4484517:mayank4484517@cluster0.rvctob4.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
...
# Subscription Plans
SUBSCRIPTION_PLANS = {
    "1day": {"price_inr": 15, "price_usd": 0.15, "days": 1, "shared_limit": 0},
    "3days": {"price_inr": 40, "price_usd": 0.40, "days": 3, "shared_limit": 0},
    "7days": {"price_inr": 80, "price_usd": 0.80, "days": 7, "shared_limit": 0},
    "1month": {"price_inr": 299, "price_usd": 3, "days": 30, "shared_limit": 0},
}

# Payment QR Code URL
PAYMENT_QR_URL = "https://t.me/MayankDon1/106"
SUPPORT_USER = "@Verifiedsynax"
CHANNEL_SUPPORT = "@S7nax"
...
def save_to_mongodb(collection, data, filter_field=None):
    if not mongo_client:
        return False
    try:
        if filter_field:
            collection.update_one(filter_field, {"$set": data}, upsert=True)
        else:
...
def get_from_mongodb(collection, query):
    if not mongo_client:
        return None
    try:
...
def has_access(user_id):
    ...
    if mongo_client:
        ...
        if not user or not user.get("access_expiry"):
            return False, None
        expiry = datetime.fromisoformat(user["access_expiry"])
...
def ban_user(user_id):
    if mongo_client:
        users_collection.update_one({"user_id": user_id}, {"$set": {"is_banned": 1}}, upsert=True)
        return
...
def unban_user(user_id):
    if mongo_client:
        users_collection.update_one({"user_id": user_id}, {"$set": {"is_banned": 0}}, upsert=True)
        return
...
def get_all_users():
    if mongo_client:
        ...
        return [(u.get("user_id"), u.get("username", ""), u.get("first_name", ""), 
                 u.get("joined_date", ""), u.get("is_banned", 0), 
                 u.get("access_expiry"), u.get("shared_id_limit", 0)) for u in users]
...
def load_campaigns(user_id):
    if mongo_client:
        campaigns = list(campaigns_collection.find({"user_id": user_id}, {"_id": 0}).sort("timestamp", -1).limit(50))
        return campaigns
...
def load_scheduled(user_id):
    if mongo_client:
        schedules = list(scheduled_collection.find({"user_id": user_id, "status": "pending"}, {"_id": 0}))
        return [(s.get("id"), s.get("action"), s.get("target"), s.get("scheduled_time"), 
                 s.get("account_count"), s.get("spam_message", "")) for s in schedules]
...
def delete_scheduled(schedule_id):
    if mongo_client:
        scheduled_collection.delete_one({"id": schedule_id})
        return
...
def update_scheduled_status(schedule_id, status):
    if mongo_client:
        scheduled_collection.update_one({"id": schedule_id}, {"$set": {"status": status}})
        return
...
def get_pending_schedules():
    if mongo_client:
        schedules = list(scheduled_collection.find({"status": "pending", "scheduled_time": {"$lte": datetime.now().isoformat()}}, {"_id": 0}))
        return [(s.get("id"), s.get("user_id"), s.get("action"), s.get("target"), 
                 s.get("scheduled_time"), s.get("account_count"), s.get("spam_message", "")) for s in schedules]
...
async def show_subscription_plans(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        [InlineKeyboardButton("⭐ 1 Day - ₹15 | $0.15", callback_data="subscribe_1day")],
        [InlineKeyboardButton("⭐ 3 Days - ₹40 | $0.40", callback_data="subscribe_3days")],
        [InlineKeyboardButton("⭐ 7 Days - ₹80 | $0.80", callback_data="subscribe_7days")],
        [InlineKeyboardButton("⭐ 1 Month - ₹299 | $3", callback_data="subscribe_1month")],
        [InlineKeyboardButton("❌ Cancel", callback_data="main")],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    text = """⚜️ VIP SUBSCRIPTION PLANS
...
async def handle_subscription(update: Update, context: ContextTypes.DEFAULT_TYPE, plan_key: str):
...
━━━━━━━━━━━━━━━━━━━━━━
Plan: {plan_key.upper()}
Price: ₹{plan['price_inr']} | ${plan['price_usd']}
Days: {plan['days']} days
━━━━━━━━━━━━━━━━━━━━━━

📌 HOW TO PAY:
1️⃣ Scan the QR code below
2️⃣ Make payment of ₹{plan['price_inr']}
3️⃣ Send payment screenshot to {SUPPORT_USER}

━━━━━━━━━━━━━━━━━━━━━━
✅ After payment confirmation, you'll get access within 24 hours.
...
Scan QR Code to Pay:"""
    ...
    keyboard = [
        [InlineKeyboardButton("💳 View QR Code", url=PAYMENT_QR_URL)],
        [InlineKeyboardButton("📞 Contact Support", url=f"https://t.me/Verifiedsynax")],
        [InlineKeyboardButton("🔙 Back to Plans", callback_data="subscription_plans")],
    ...
    # Store pending payment for manual approval
...
async def profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mongo_client:
        ...
        joined_date = user.get("joined_date", "Unknown") if user else "Unknown"
        access_expiry = user.get("access_expiry") if user else None
        shared_limit = user.get("shared_id_limit", 0) if user else 0
    else:
        conn = sqlite3.connect('automation_bot.db')
        c = conn.cursor()
...
👨‍💻 Channel: {CHANNEL_SUPPORT}
📞 Access/Support: {SUPPORT_USER}
🔧 Bot Owner: {SUPPORT_USER}
💳 Payment/QR: {PAYMENT_QR_URL}

For issues, bugs, or access requests, contact above."""
    
    keyboard = [
        [InlineKeyboardButton("📢 Join Channel", url=f"https://t.me/{CHANNEL_SUPPORT[1:]}")],
        [InlineKeyboardButton("👨‍💻 Contact Support", url=f"https://t.me/Verifiedsynax")],
        [InlineKeyboardButton("💳 View QR", url=PAYMENT_QR_URL)],
        [InlineKeyboardButton("BACK", callback_data="main")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main":
        ...
        await handle_subscription(update, context, plan_key)
...
async def main():
    ...
    print("=" * 50)
    print("✅ MongoDB: Connected" if mongo_client else "⚠️ MongoDB: Not Connected (using SQLite)")
    print("=" * 50)
    ...
    print("   - 1 Day: ₹15 | $0.15")
    print("   - 3 Days: ₹40 | $0.40")
    print("   - 7 Days: ₹80 | $0.80")
    print("   - 1 Month: ₹299 | $3")
    print(f"   - Payment QR: {PAYMENT_QR_URL}")
    print(f"   - Support: {SUPPORT_USER}")
    print(f"   - Channel: {CHANNEL_SUPPORT}")
    print("=" * 50)
